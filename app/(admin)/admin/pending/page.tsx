...
export default async function AdminPendingPage() {
  const [pendingStartups, pendingOrgs] = await Promise.all([
    getAllStartupsAdmin("pending"),
    getAllOrganizationsAdmin("pending"),
  ]);

  const total = pendingStartups.length + pendingOrgs.length;
    ...
    <div>
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-neutral-500">{total} submission{total !== 1 ? "s" : ""} awaiting review</p>
        </div>
      </div>
        ...
        <div className="space-y-6">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-start gap-4">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          <h3 className="font-semibold text-neutral-900 text-lg">{startup.name}</h3>
          ...
          {pendingOrgs.length > 0 && (
            <div>
              ...
              <div className="space-y-4">
                {pendingOrgs.map((org) => (
                  <div key={org.id} className="bg-white rounded-2xl border border-neutral-100 p-6">
                    <div className="flex flex-col sm:flex-row sm:items-start gap-4">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          <h3 className="font-semibold text-neutral-900 text-lg">{org.name}</h3>
                          <span className="text-xs bg-yellow-100 text-yellow-700 px-2 py-0.5 rounded-full font-medium">Pending</span>
                        </div>
                        <p className="text-sm text-neutral-500 mb-3">{org.tagline}</p>
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs text-neutral-500 mb-4">
                          <div><span className="font-medium text-neutral-700">Type:</span> {org.orgType ?? "—"}</div>
                          <div><span className="font-medium text-neutral-700">LGA:</span> {org.lga ?? "—"}</div>
                          <div><span className="font-medium text-neutral-700">Submitted:</span> {formatDate(org.createdAt.toISOString())}</div>
                        </div>
                        {org.description && (
                          <p className="text-sm text-neutral-600 line-clamp-3 bg-neutral-50 p-3 rounded-lg">
                            {org.description}
                        ...
                        {(org as any).owner && (
                          <div className="mt-3 text-xs text-neutral-500">
                            Submitted by: <span className="font-medium">{(org as any).owner.name}</span>{" "}
